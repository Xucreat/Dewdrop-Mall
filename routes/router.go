...
func NewRouter() *gin.Engine {
	...
	v1 := r.Group("api/v1")
	{
		...
		// 商品操作
		v1.GET("/list-products", api.ListProducts)
		v1.GET("/product/:id", api.ShowProduct)
		v1.POST("/search-products", api.SearchProducts)
		...
		authed.Use(middleware.JWT())
		{
			...
			// // 收藏夹
			// authed.GET("favorites", api.ShowFavoriteStores)
			// authed.POST("favorites", api.CollectFavoriteStore)
			// authed.DELETE("favoritestore/:id", api.DeleteFavoriteStore)
			// authed.DELETE("favorites/batch", api.DeleteFavoriteStores)
			// 前端希望删除 boss_id 为 1, 2 和 3 的店铺收藏
			// {
			// 	"boss_ids": [1, 2, 3]
			// }
