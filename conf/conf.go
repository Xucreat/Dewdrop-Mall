...
func Init() {
	// 从本地读取环境变量
	file, err := ini.Load("/home/bx/code_files/bx-mall/conf/config.ini")
	if err != nil {
	...
	LoadRabbitMQ(file)
	LoadRedisData(file)
	if err := LoadLocales("/home/bx/code_files/bx-mall/conf/locales/zh-cn.yaml"); err != nil {
		panic(err)
	}
}
...
func LoadMysqlData(file *ini.File) {
	...
	DbHost = file.Section("mysql").Key("DbHost").String() // 主数据库地址
	DbPort = file.Section("mysql").Key("DbPort").String()
	DbUser = file.Section("mysql").Key("DbUser").String()
	DbPassWord = file.Section("mysql").Key("DbPassWord").String()
	DbName = file.Section("mysql").Key("DbName").String()
	// log.Println("Loaded DB Config: ", DbHost, DbPort)
	// 加载从数据库（读操作）配置
	// DbReadHost = file.Section("mysql").Key("DbReadHost").String() // 从数据库地址
}
